...
const BLOG_STYLE_NOTES = [
  ...
  "  노래 하나에 얽힌 성민님의 개인적인 이야기를 풀어내는 긴 글입니다.",
  "  소개하는 곡은 다른 아티스트의 곡일 수도, 예명 '수아'로 직접 작사·노래해 발매한 본인 곡일 수도 있습니다 —",
  "  '추천곡이냐 자작곡이냐'로 미리 나누지 말고, 그때그때 자연스럽게 어울리는 이야기를 쓰세요.",
...
export async function generateBriefingIdeas(env: AskEnv): Promise<AskResult> {
  const system = [
    ...
    "'성민의 하루 플레이리스트'와 '오늘의 음악' 아이디어를 섞어서,",
    "각각 '시리즈', '곡·주제', '왜 지금 이 얘기를 하면 좋은지 한 줄'을 목록으로 작성하세요.",
...
export async function generateBlogDraft(env: AskEnv): Promise<AskResult> {
  const system = [
    ...
    "먼저 '성민의 하루 플레이리스트'와 '오늘의 음악' 중 어느 시리즈로 쓸지 자유롭게 고르고, 그 구조를 정확히 따르세요.",
    "형식은 정확히 아래처럼 맞추세요.",
    "",
    "시리즈: 플레이리스트 또는 오늘의음악 중 하나만 적으세요",
    "자작곡: 이 글에서 소개하는 곡이 예명 '수아'의 본인 발매곡이면 예, 다른 아티스트의 곡이면 아니요",
    "제목: (해당 시리즈의 제목 형식을 그대로 따른 구체적인 제목)",
    "",
    "(본문 — 고른 시리즈의 구조·소제목·분량을 BLOG_STYLE_NOTES 그대로 따르세요)",
